...
2#  Ok, let's start from the beginning. Immutability is when you create something that cannot be changed anymore. For example, an immutable server is a server you put in your cloud, and which no one can change anymore, for any reason.
1# Doesn’t seem so useful.
2# It seems so, indeed, but follow me.
...
1# I cannot see what the advantage is, honestly…
...
2# If we could print 20 copies out of the Master Copy, what would Anna’s task be?
...
2# Yes, absolutely. Every configuration would be as simple as a text file, stored in the Master Copy, for all the System Engineers to read and manage. Anyone who had worked on the Master Copy can manage anyone of its copies.
